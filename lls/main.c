...
lls_stage2(void *arg)
{
	...
	if (lls_conf->arp_cache.iface_enabled(net_conf, &net_conf->front)) {
		...
		if (ret < 0 && net_conf->front.rss &&
			 ...
			 */
			LLS_LOG(ERR, "If EtherType flows are not supported, the LLS block must listen on queue 0 on the front iface\n");
			return -1;
		} else if (ret >= 0) {
	...
	if (lls_conf->arp_cache.iface_enabled(net_conf, &net_conf->back)) {
		...
		if (ret < 0 && net_conf->back.rss &&
			...
			LLS_LOG(ERR, "If EtherType flows are not supported, the LLS block must listen on queue 0 on the back iface\n");
			return -1;
		} else if (ret >= 0) {
			/* ARP packets can be received from the NIC. */
			lls_conf->rx_method_back |= RX_METHOD_NIC;
